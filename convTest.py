...
class config:
    ...
    def setting_name(self):
        return 'doc2vec_cov2d_'+'ds-'+self.dataset_path +'optimizer-'+self.optimizer+'loss-'+str(self.loss)
...
doc2vec_configs = [
                   ...
                   # config( 'imdb.d2v', 'RMSProp', 'mean_square'),
                   config( 'imdb.d2v', 'Momentum', 'mean_square'),
...
for cl in doc2vec_configs:
    with tf.Graph().as_default():
        print("running doc2vecRegression_conv2d_on_"+cl.setting_name())
        ...
        trainX=numpy.expand_dims(trainX, -1)
        trainX=numpy.expand_dims(trainX, -1)
        ...
        testX=numpy.expand_dims(testX, -1)
        testX=numpy.expand_dims(testX, -1)

        testY = to_categorical(test_labels, nb_classes=2)
        # Network building
        
        net = tflearn.input_data(shape=[None, 100,1,1])
        net = tflearn.conv_2d(net, 32, [20,1], activation='relu', name='conv1')
        net = tflearn.conv_2d(net, 32, [5,1], activation='relu', name='conv1')
